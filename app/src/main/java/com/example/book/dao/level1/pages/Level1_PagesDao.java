...
public interface Level1_PagesDao {
    ...
    @Query("select * from level1_pages where bookName=:book order by chapter")
    LiveData<List<Level1_Pages>> getChapters(String book);

    @Query("select chapterName from level1_pages where bookName=:book order by chapter")
    LiveData<List<String>> getChapterNames(String book);
